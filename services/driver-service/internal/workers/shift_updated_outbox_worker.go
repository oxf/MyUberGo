...
var tracer = otel.Tracer("driver-service/outbox")
...
// MaxRetries exports defaultMaxRetries for cmd/main.go's outbox backlog gauges, so the
// "parked" threshold there can never silently drift from what this worker enforces.
const MaxRetries = defaultMaxRetries

// OutboxWorker drains driver.outbox_message and publishes each row to Kafka.
...
// publishOne is scoped to a single outbox row so its span ends via defer even on panic — a bare
// span.End() in processBatch's loop would never run in that case. Its error still aborts the batch on failure.
