// Package decorator wires auth-service's own error classification into the
...
// isExpectedError classifies known domain errors (bad password, expired
// token, duplicate signup) as expected outcomes rather than defects for span status.
func isExpectedError(err error) bool {
	return errors.Is(err, commonerrors.ErrNotFound) ||
		errors.Is(err, commonerrors.ErrInvalidCredentials) ||
		errors.Is(err, commonerrors.ErrInvalidToken) ||
