// Package decorator wires matching-service's own error classification into the
...
// isExpectedError classifies a lost accept race or expired/cancelled offer as expected
// business outcomes, not defects, so they don't inflate the span-status error-rate ratio.
func isExpectedError(err error) bool {
	return errors.Is(err, commonerrors.ErrNotFound) ||
		errors.Is(err, commonerrors.ErrOfferGone) ||
		errors.Is(err, commonerrors.ErrRideTaken)
}
