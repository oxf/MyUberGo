...
- `GET /users`, `GET /driver`, and `GET /driver-shift` are Admin-only at the Kong gateway now (see `gateway/kong.yml`). The simulator logs in once at startup as the seeded admin (`services/shared/migrations/init.sql`) and shares that token (`Deps.AdminAccessToken`) across every actor's list-endpoint verify — an ordinary client/driver token gets a 403 on these three routes.
