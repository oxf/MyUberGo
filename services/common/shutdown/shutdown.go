...
// OnStop registers a function called once shutdown begins, before waiting on the WaitGroup —
// typically a context cancel func telling background workers to stop.
func (m *Manager) OnStop(fn func()) {
	m.onStop = append(m.onStop, fn)
}

// OnDrained registers fn to run after every OnStop hook fired AND the WaitGroup reached zero (or
// timed out) — register provider flush/close here, never via OnStop, or it drops in-flight telemetry.
func (m *Manager) OnDrained(fn func()) {
	m.onDrained = append(m.onDrained, fn)
}
