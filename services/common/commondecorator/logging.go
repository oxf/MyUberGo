...
func logOutcome(logger *logrus.Entry, err error, isNotFound func(error) bool, successMsg, notFoundMsg, failureMsg string) {
	switch {
	case err == nil:
...
func (d commandLoggingDecorator[C, R]) Handle(ctx context.Context, cmd C) (result R, err error) {
	fields := logrus.Fields{"command": ActionName(cmd)}
	...
	defer func() {
		logOutcome(logger, err, d.isNotFound,
...
func (d queryLoggingDecorator[C, R]) Handle(ctx context.Context, cmd C) (result R, err error) {
	fields := logrus.Fields{"query": ActionName(cmd)}
	if id, ok := idField(cmd); ok {
		fields["query_id"] = id
	}
	logger := loggerFromContext(ctx, d.logger).WithFields(fields)
