...
func TestInsert_MarkProcessed_IncrementRetries(t *testing.T) {
	...
	var id string
	if err := testDB.QueryRow(`SELECT id FROM ride.outbox_message WHERE topic = $1`, topic).Scan(&id); err != nil {
		t.Fatalf("find inserted row by topic: %v", err)
	}

	var processed bool
	var retries int
	if err := testDB.QueryRowContext(ctx, `SELECT processed, retries FROM ride.outbox_message WHERE id = $1`, id).Scan(&processed, &retries); err != nil {
	...
	if err := testDB.QueryRowContext(ctx, `SELECT retries FROM ride.outbox_message WHERE id = $1`, id).Scan(&retries); err != nil {
	...
	if err := testDB.QueryRowContext(ctx, `SELECT processed FROM ride.outbox_message WHERE id = $1`, id).Scan(&processed); err != nil {
