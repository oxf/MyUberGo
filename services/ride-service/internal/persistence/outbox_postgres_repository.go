...
func (r *PostgresOutboxRepository) SetClaimedUntil(
	...
	_, err := executor.ExecContext(
		ctx,
		`UPDATE ride.outbox_message SET claimed_until = $2 WHERE id = $1`,
		id,
		claimedUntil,
	)
...
func (r *PostgresOutboxRepository) MarkProcessed(
	...
	_, err := executor.ExecContext(
		ctx,
		`UPDATE ride.outbox_message SET processed = true WHERE id = $1`,
		id,
	)
...
func (r *PostgresOutboxRepository) IncrementRetries(
	...
	_, err := executor.ExecContext(
		ctx,
		`UPDATE ride.outbox_message SET retries = retries + 1 WHERE id = $1`,
		id,
	)
