...
**Schema**: `RIDE` (client/driver ids, status machine `Requested → Matched → InProgress → Completed`/`Cancelled`/`Failed`, timestamps per transition, `bill_id`, `rating_id`), `RIDE_REQUEST` (pickup/destination geometry, distance, price, tariff), `TARIFF` (time-of-day pricing: Morning/Noon/Evening/Weekend), `OUTBOX_EVENTS`.
...
**Kafka subscribes**: `ride.accepted` (→ set driver/matched_at/status), `ride.completed` (→ set bill_id, *renamed from `ride.finished` above — that topic name doesn't exist*).

> Current code implements `POST /request-ride`, `GET /ride`, `GET /ride/{id}`, `DELETE /ride/{id}` (cancel), plus two driver-triggered endpoints the target list above doesn't mention — `POST /ride/{id}/start`, `POST /ride/{id}/complete` (no `/api` prefix, different route names than the target above) — with a flat `ride.ride` table. `ride.tariff` is now read: fare is `base_fare_minor + price_per_km_minor*distanceKm + price_per_min_minor*durationMin` (haversine distance, an assumed average speed for duration), all in integer minor units + an ISO-4217 `currency` — see CLAUDE.md's money-representation invariant. `POST /request-ride` accepts an optional `tariffName` (defaults `"Standard"`); a `"Standard USD"` tariff is also seeded, so multi-currency is exercisable end-to-end. Publishes `ride.requested`/`ride.started`/`ride.completed`/`ride.cancelled` (all carrying `clientId` + the money fields billing-service needs) and consumes `ride.accepted` (→ `Matched`), `payment.completed` (→ sets `bill_id`), and `ride.matching_failed` (→ `Failed`, published by matching-service when it gives up retrying — see §3 below). Ride status now has a fifth terminal value, `Failed`, alongside `Requested`/`Matched`/`InProgress`/`Completed`/`Cancelled`. Cancellation fee is a stub flat 300 minor units, charged only when a driver was already assigned — real per-distance/per-time fee logic is still future work, but the path is no longer permanently dead.

### 3. Matching Service — ✅ Implemented, simplified algorithm (Port `:8002`)

Consumes ride/shift/cancellation events and matches ride requests to available drivers. Target design is a full radius-search + weighted-ranking + tiered-broadcast + atomic-accept + expanding-retry algorithm (see [Matching Algorithm](#-matching-algorithm-target-design) below); current code implements most of that shape already — geo-first discovery with real expanding-radius retry, distance+rating ranking (not rating-only), atomic accept, concurrent offer fan-out — with **BROADCAST-only** (no TIERED escalation) as the one remaining structural simplification. Backed entirely by Redis (no Postgres — matching-service has never had a Postgres dependency in the live code path, and it was formally removed from `go.mod`/`docker-compose.yml`).
...
**Kafka publishes**: `ride.accepted` (published directly from the accept handler, no outbox — Redis has no transaction to hide a dual write behind, so a crash between the Redis match and the Kafka publish loses the event; an accepted at-most-once tradeoff since the match itself is durable in Redis) and `ride.matching_failed` (published, same at-most-once tradeoff, when a ride exhausts all 5 retry attempts — closes a loop that used to be silent: ride-service consumes it and flips the Postgres row to `Failed`).

> Simplifications vs. the target design below: **discovery is geo-first with a rating-only fallback** (changed 2026-08-12, once the Location service's Slice 1 shipped) — `BroadcastOffersHandler` calls Location's `GET /internal/drivers/nearby` for geographic candidates, intersects against `drivers:online` (Location doesn't track shift state), and ranks by `0.5×distance + 0.5×rating` (README's `acceptance_rate` term has no data source yet); any failure in that path falls back to the original rating-only `drivers:online` ZSET query, never failing the ride. `driver.rating` is still carried on an enriched `shift.updated` event either way; a driver enters the pool on `status: "Online"` and leaves it on any other status (including `"Ended"` — this used to be silently dropped by a driver-service bug that's now fixed, see below). **Broadcasting is BROADCAST-only** (top 5 at once, first accept wins) — TIERED escalation is the one remaining broadcast-strategy gap; the offer-send step itself *is* already concurrent (fanned out via `errgroup`, not sequential). **Retry expands the geo search radius** (5→7→9→11→13km, cap 15) when Location is available, falling back to pool-widening only (`attempt × 5` drivers) on the rating-only path — capped at 5 attempts via a `MatchRetryWorker` background sweep (ticker+select loop over `pending_ride:*`, itself still a sequential per-ride loop — see `PLAN.md`); giving up marks the ride `failed` in Redis and publishes `ride.matching_failed` (see above) so the client-visible ride status actually changes, not just a log line. **Rate limiting** is a sliding 60s window (each offer resets the TTL), not a strict fixed window, but caps at 3/minute as designed. `ride.cancelled` is now produced by ride-service and consumed here (see above) — the `ride:{id}:cancelled` key is actually set, not just checked. `ride-service` now consumes `ride.accepted` too (see the Ride Service section above), so a matched ride's Postgres status does flip to `Matched`.
...
**HTTP (target)**: `POST /api/shifts/start`, `POST /api/shifts/end`, `GET /api/drivers/available?lat&lon&radiusKm` (internal, used by Matching). *(The target design below originally put ride start/finish here too — in the actual build that responsibility landed on ride-service instead, as driver-triggered endpoints; see §2 above.)*

**Kafka publishes (target)**: `shift.started`, `shift.ended`. *(These specific topic names don't exist in the current build — see the note below for what does.)*
**Kafka subscribes (target)**: `ride.accepted` (→ status=Assigned).

> Current code implements `POST /driver`, `PUT /driver/{id}`, `GET /driver`, `GET /driver/{id}`, `POST /driver-shift/create`, `PUT /driver-shift/{id}`, `GET /driver-shift`, `GET /driver-shift/{id}` (no `/api` prefix, different route/table names than the target above — the table is `driver.driver`, renamed from `driver_profile`) — driver CRUD and shift create/update, with a working transactional-outbox worker publishing a single `shift.updated` event to Kafka (not the target's separate `shift.started`/`shift.ended`; carries the driver's `rating`, used by matching-service's ranking as the fallback input when geo discovery is unavailable — see below). `driver.driver` no longer stores `name`/`phone` either (dropped in favor of `auth.user` as the single source — see the Auth Service section above); `GET /driver`/`GET /driver-shift` are Admin-only at the Kong gateway. It doesn't yet have license/vehicle detail fields, `acceptance_rate`/`current_shift_id`, or the `available` nearby-driver endpoint (matching-service gets geo discovery from location-service instead, not from driver-service — see §5 below). **`driver.driver.status` has three real values** (`Offline`/`Online`/`OnRide`, not the two this section implied), and the three ride-lifecycle consumers (`ride.accepted`, `ride.completed`, `ride.cancelled`) are **not placeholders** — they persist real state: `Online→OnRide` on accept, `OnRide→Online` (+ increments `total_rides_completed`) on completion, `OnRide→Online` on cancellation, each guarded so a redelivered event is a harmless no-op. A bug where setting a shift to `Ended` never published a `shift.updated` event at all (the handler returned before reaching the outbox insert) is fixed — matching-service's online-driver pool now correctly shrinks when a driver goes offline.
...
The full design for `matching-service`'s ride-to-driver matching. Current code implements most of this shape already (see the "Matching Service" section above) — the two remaining gaps are the `acceptance_rate` ranking term (no data source anywhere yet) and TIERED broadcast escalation (only the simpler BROADCAST strategy is implemented).
...
The infrastructure runs locally via Docker Compose (`docker-compose.yml`):
*   ✅ **PostgreSQL 15**: primary relational storage shared across services (separate schemas per service), plus a one-shot `migrate` service (golang-migrate) that applies `services/shared/migrations/sql/` before the app services start.
*   ✅ **Redis 7 (Alpine)**: current store for Matching's offer/accept state and Location's live GPS cache — not a target, both already use it.
*   ✅ **Apache Kafka** + **Kafka UI** (`:8080`): event streaming and a web dashboard for inspecting topics/consumer groups/messages.
*   ✅ **API Gateway — Kong** (`:8090`, mapped from Kong's internal `:8000`): the sole ingress for all 6 business services.
*   ✅ **Admin dashboard — `web/`** (`:5173`): a Vite+React+TS read-only dashboard (Users/Drivers/Shifts/Rides/Invoices), proxying `/api/*` to Kong.
*   ✅ **Observability stack**: OTel Collector (traces/metrics/logs ingest) → Tempo (`:3000` via Grafana, traces), Prometheus (`:9090`, metrics), Loki (logs), Alertmanager (`:9093`, SLO/operational alert routing), Grafana (`:3000`, anonymous Admin locally, 4 provisioned dashboards) — config under `observability/` at the repo root.
*   📋 **DynamoDB/Cassandra** *(planned)*: time-series store for Location Service's GPS history (Slice 3, not yet in `docker-compose.yml`).

Two more pieces live outside docker-compose, run manually against the stack above: **`services/e2e-test`** (continuous client/driver simulator + a one-shot scenario mode) and the **Bruno API collection** (`MyUberGo/` at the repo root). **`services/common`** and **`services/observability`** are shared Go modules (not compose services) — every business service imports them for CQRS decorators, DB/Redis bootstrap, Kafka consumer/publisher/outbox plumbing, and the full OpenTelemetry wiring; see `CLAUDE.md` for the package inventory.
...
All 6 implemented services are fully in Phase 2 now, including `auth-service` (refactored 2026-07-25 — see Phase 1 of the roadmap below) and `billing-service`/`location-service` (Phase 2 from day one, never Phase 1 at all).

---

## 🚀 Development & Refactoring Roadmap

> For what's currently open — grouped by area, not phase-ordered — see **[PLAN.md](./PLAN.md)**. The phases below are the high-level roadmap; for the dated history of how each phase got built (bugs found, decisions made, what was verified), see `docs/CHANGELOG.md`.

### Phase 1: Clean Architecture Refactoring (Core Services)
*   [x] `driver-service` refactored to CA/CQRS.
*   [x] `matching-service` refactored to CA/CQRS + matching algorithm implemented (discovery/broadcast/atomic accept/retry/rate limiting against Redis). Geo-radius discovery landed 2026-08-12 (Location service Slice 1) — ranking is now `0.5×distance + 0.5×rating` when Location is available, rating-only on fallback; the offer-broadcast fan-out is concurrent (`errgroup`), not sequential. Remaining: TIERED broadcast strategy; `MatchRetryWorker`'s per-ride retry sweep is still sequential (see `PLAN.md`).
*   [x] **Refactor `ride-service`**: fully Stage 2 (CQRS/DDD layered) and the `Cancelled`/`Failed` lifecycle (`DELETE /ride/{id}`, `ride.cancelled`/`ride.matching_failed` publish/consume) is implemented — see the Ride/Matching/Driver Service sections above. `ride.tariff` **is** now read (fare calculation is tariff-driven); still remaining: a separate `RIDE_REQUEST` table and time-of-day pricing (current tariffs are flat rates, not Morning/Noon/Evening/Weekend-windowed), and a real (non-stub) cancellation fee calculator — Billing exists now, but the stub flat-fee logic hasn't been replaced with real per-distance/per-time calculation yet.
...
*   [x] Implement OpenTelemetry tracing to trace requests end-to-end from the Gateway through PostgreSQL and Kafka — Kong originates the trace via its `opentelemetry` plugin, all 6 services propagate it (including through the transactional outbox, via a persisted `trace_context` column — see CLAUDE.md's "Observability (OpenTelemetry)" section). Traces land in Tempo, viewable in Grafana at `http://localhost:3000`.
...
1.  **Spin up infrastructure and services**:
    ...
    ```
    Migrations (`services/shared/migrations/sql/`, golang-migrate) apply automatically via a one-shot `migrate` compose service before the Postgres-backed app services start — no manual step needed.

2.  **Call the API**. Kong (`http://localhost:8090`) is the *only* entry point — none of the 6 business services publish a host port. Every request goes through `http://localhost:8090/api/{auth,ride,driver,matching,billing,location}/...`. Log in as the seeded local admin to explore Admin-only routes:
    ```bash
    curl -X POST http://localhost:8090/api/auth/login \
      -H "Content-Type: application/json" \
      -d '{"email":"admin@myubergo.local","password":"admin123"}'
    ```

3.  **Explore visually**:
    *   **Admin dashboard** — `http://localhost:5173` (Users/Drivers/Shifts/Rides/Invoices, log in as the seeded admin above)
    *   **Kafka UI** — `http://localhost:8080` (topics, consumer groups, messages)
    *   **Grafana** — `http://localhost:3000` (anonymous Admin locally; traces/metrics/logs, 4 provisioned dashboards)
    *   **Bruno collection** — `MyUberGo/` at the repo root, one folder per service, a `Local` environment pointed at Kong

4.  **Manual testing beyond Bruno**: `services/e2e-test` is a continuous client/driver simulator (`go run ./cmd` from that directory, not part of docker-compose) that drives the full ride lifecycle end-to-end with deep read-back verification — see its README for config and its scenario mode.
