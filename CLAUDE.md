...
| billing-service | 8005 | Invoices completed/fee-bearing rides, collects via a pluggable `PaymentProvider` (stub only, real Stripe deferred), records a double-entry ledger; consumes `ride.completed`/`ride.cancelled`, publishes `payment.completed`/`payment.failed` |
...
| billing-service | Stage 2 from day one | full CQRS/DDD layering copying ride-service's shape; two transactional-outbox-style workers (`OutboxWorker` for `payment.completed`/`payment.failed`, and a `ChargeWorker` sweeping open invoices through a stub `PaymentProvider`); the double-entry ledger (`ledger_account`/`ledger_transaction`/`ledger_entry`) is the one genuinely new architectural piece not mirrored from another service — see "Data model" above and `docs/billing/BILLING_SPEC.md`. Real Stripe integration deliberately deferred (spec §9). |
