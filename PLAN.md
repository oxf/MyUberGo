...
- [ ] **Test coverage**: every service has zero tests for `persistence`, `consumers`, and (except billing's `ChargeWorker`) `workers` packages — the layers most likely to break silently. Start with `ride-service` and `billing-service` (money-handling).
  ...
  - [ ] **Consumers and workers (both services), deferred**: not attempted this pass. `ride-service`'s Kafka consumers and `billing-service`'s consumers/remaining `outbox_worker.go` still have zero tests. The same testcontainers-go Postgres harness carries over directly; consumer tests additionally need a real or faked Kafka broker (`segmentio/kafka-go`), a second infra decision deliberately left for that follow-up pass.
