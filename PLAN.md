...
### Phase 1 — foundational safety nets (open)

- [ ] **CI**: add `.github/workflows` — matrix job per Go service (`go build ./...`, `go vet ./...`, `go test ./...`) + a `web` job (`npm run build`, `oxlint`). Zero CI exists today; nothing gates `main`. Add a root `.golangci.yml` and wire `golangci-lint run` into the same workflow.
- [ ] **Secret hygiene**: `secret_change_me`/`postgres:postgres` are duplicated across `docker-compose.yml` (4x JWT, 4x DSN) and `gateway/kong.yml` with no single source of truth. Centralize into `.env` (extend `.env.example` past just the Stripe vars) and add a refuse-to-boot-in-prod check if `JWT_SECRET` is still the default (mirror the existing Stripe `sk_test_`-only guard).
- [ ] **Panic recovery**: no `recover()` exists anywhere in the codebase. Add a `recover()`-wrapping HTTP middleware (alongside the new `BodyLimit`/`RequestID` chain) and wrap the ~15 long-lived worker/consumer goroutines per service so one bad message doesn't kill the whole process.
- [ ] **Context timeouts on consumer-triggered work**: Kafka consumers call their `handle*` function with the raw background context (no deadline) — only the outbox *publish* side has a timeout today. Wrap each consumer's handler call in `context.WithTimeout`.
- [ ] **Outbox dead-letter / retry cap**: `retries` is incremented in `outbox_message` but never read anywhere — a permanently-broken message retries forever at a fixed 2s cadence. Add a max-retry cutoff that parks/logs instead, reusing billing-service's `ChargeWorker` tiered-backoff as the reference pattern.
- [ ] **docker-compose hardening**: add `healthcheck:` blocks for the 5 Go services (they already expose `/health/live`), `restart: unless-stopped`, and basic `mem_limit`/`cpus` ceilings (Kafka especially — it currently has none).
- [ ] **Migration tooling**: adopt `golang-migrate` or `goose` going forward; keep `init.sql` as the frozen baseline for fresh DBs, add numbered migrations from here on instead of continuing to hand-edit one 373-line file.
- [ ] **Error responses**: stop leaking raw `err.Error()` strings to HTTP clients across all 5 services (info disclosure) — log server-side, return a generic message.
- [ ] **Kong route for matching-service**: add a proper Kong route (with rate-limiting) instead of leaving `:8002` published unauthenticated — see the Phase 0 note above.
